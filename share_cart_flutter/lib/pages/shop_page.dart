    ...
    return Scaffold(
      ...
      body: Consumer<GroupDetailsProvider>(builder: (context, groupDetailsProvider, _) {
        ...
        List<ShareCartItem> filteredItems = items
            .where((item) =>
                item.name.toLowerCase().contains(_searchQuery) || item.category.toLowerCase().contains(_searchQuery) || item.description.toLowerCase().contains(_searchQuery))
            .toList();
        ...
        return RefreshIndicator(
          ...
          child: Center(
            ...
            Padding(
                ...
                child: Column(children: [
                  TextField(
                    decoration: InputDecoration(border: OutlineInputBorder(), labelText: 'Query'),
                    onChanged: (value) {
            ...
            Expanded(
                child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ShareCartItemDetailsWidget(
                          item: item,
                          onTap: () async {
                            final newListItem = await showDialog<AddItemToList>(context: context, builder: (_) => AddItemToListDialog(item: item));
